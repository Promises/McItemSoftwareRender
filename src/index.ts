...
function orthographicProject(vertex: Vector3): Vector2 {
    return {
        x: vertex.x + vertex.z, // Example of adjusting x to include some z-depth effect
        y: vertex.y
    };
}
...
class Model {
        ...
        for (let y = minY; y <= maxY; y++) {
            for (let x = minX; x <= maxX; x++) {
                if (this.pointInPolygon(x, y, vertices)) {
                    ...
                    const textureX = Math.floor(uv.x * face.texture.width);
                    const textureY = Math.floor(uv.y * face.texture.height);
    ...
    interpolateUV(x: number, y: number, vertices: Vector2[], uv: number[]): Vector2 {
        ...
        const triangle1Area = this.triangleArea(A, B, C);
        const triangle2Area = this.triangleArea(A, C, D);

        let wA, wB, wC, wD;
        if (triangle1Area > triangle2Area) {
            // Point is in triangle ABC
            wA = this.triangleArea(B, C, {x, y}) / triangle1Area;
            wB = this.triangleArea(C, A, {x, y}) / triangle1Area;
            wC = this.triangleArea(A, B, {x, y}) / triangle1Area;
            wD = 0; // Not in triangle ACD
        } else {
            // Point is in triangle ACD
            wA = this.triangleArea(C, D, {x, y}) / triangle2Area;
            wC = this.triangleArea(D, A, {x, y}) / triangle2Area;
            wD = this.triangleArea(A, C, {x, y}) / triangle2Area;
            wB = 0; // Not in triangle ABC
        }

        // Interpolate UV using barycentric weights
        const interpolatedU = wA * uv[0] + wB * uv[1] + wC * uv[2] + wD * uv[3];
        const interpolatedV = wA * uv[0] + wB * uv[1] + wC * uv[2] + wD * uv[3]; // Assuming V is the same as U
